...
bag_rs <- fit_resamples(bag_workflow,
              resamples = stock_folds) # no need to save prediction for plotting important variables because the function is not avaiable for this model and the bag tree model has important variables automatically generated.
...
**The bag tree model performed very consistently on the training set, which achieves an accuracy of 0.883.**
...
```{r}
# butcher the workflow to make it small by removing everything else except model
# for prediction
stock_wf_model <- butcher::butcher(stock_fit$.workflow[[1]])
```

```{r}
# use lobster to check object size
lobstr::obj_size(stock_fit$.workflow[[1]])
lobstr::obj_size(stock_wf_model) # butchered one is much smaller
```
...
```{r}
predict(stock_wf_model, stock_test[121:131, ]) # use 10 obs
```

# Write model

```{r, eval=FALSE}
write_rds(stock_wf_model, "stock_wf_model.rds")
```
